...
namespace FluentMigrator.NHibernateGenerator.SF {
    public sealed class MigrationCreator {
        ...
        public void Create(string name) {
            ...
            using (StreamWriter outputFile = new (Path.Combine(dir, $"{migration.FileNamePrefix}{migration.Name}.cs")))
            {
                outputFile.Write(migration.Code);
            }
            using (StreamWriter outputFile = new (Path.Combine(dir, $"{migration.FileNamePrefix}{migration.Name}.Designer.cs")))
            {
